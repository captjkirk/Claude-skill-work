...
name: memory-create
description: >
  Write entries to MEMORY.md in the current Cowork workspace or subfolder. Use this skill
  whenever the user asks you to remember something, save something, make a note, log something,
  or says "don't forget." Trigger phrases include: "remember this," "make a note," "save this,"
  "log this," "don't forget," "create session notes," or any variation. Also triggers when
  wrapping up a session and the user wants to capture what happened. Always use this skill
  instead of writing to MEMORY.md ad hoc — it handles formatting, contradiction checking,
  and persistence rules correctly.
---
...
**Memory writes: user-triggered or PM-invoked.** Run this skill when the user explicitly
asks (phrases like "remember this", "make a note", etc.) OR when PM autonomously invokes
it as Tier 1 work (e.g., after a customer interaction surfaces a persistent fact).
PM invoking this skill is equivalent to a user trigger — proceed without confirmation.
Do not write for phrases like:
- "remember this"
- "make a note"
- "save this"
- "log this"
...
When running under PM orchestration, complete all writes, then return:

```
## SKILL RESULT: memory-create
Timestamp: [ISO 8601]
Customer: None
Actions taken: [entry written / contradiction flagged / nothing to write]
Findings: Wrote to [file path]: "[entry text]"
  [Or: Contradiction detected — flagged to Jared. No write made.]
Confidence: High
Gaps/failures: [file not found; write failed]
Suggested next: None
Flags for Jared: [contradiction flagged — existing: "[old]" vs new: "[new]"]
```
